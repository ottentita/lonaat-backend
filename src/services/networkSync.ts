...
export async function getNetworkStatus(): Promise<{network: string; configured: boolean; key_name: string; sync_type: string; missing?: string; disabled?: boolean}[]> {
  ...
  const digistoreConfigured = !!(process.env.DIGISTORE_API_KEY || process.env.DIGISTORE_PRODUCT_ID);
  
  return [
    { network: 'digistore24', configured: digistoreConfigured, key_name: 'DIGISTORE_API_KEY', sync_type: 'api' },
