...
export async function getAIRecommendations(userId: number) {
  // Minimal implementation for build stability
  const rankings = await rankProducts(userId, 10)
  return rankings
}
...
export async function rankProducts(
  ...
  const conversions = await prisma.commissions.groupBy({
    by: ["product_id"],
    _sum: { amount: true },
    _count: { id: true },
    where: {
  ...
  const conversionsMap: Map<number, { count: number; amount: number }> = new Map(
    conversions.map((c) => [
      c.product_id as number,
      { count: c?._count?.id ?? 0, amount: c._sum.amount ? Number(c._sum.amount) : 0 },
    ]),
  );
...
export async function generateGrowthReport(userId: number): Promise<GrowthReport> {
  // Mock implementation - would analyze real data in production
  try {
    ...
    return {
      overallScore: 0,
      topProducts: [],
      roiAnalysis: { currentROI: 0, projectedROI: 0, recommendations: [] },
      generatedAt: new Date(),
    };
  }
}
