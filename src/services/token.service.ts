...
async function getAdWallet(userId: number) {
  ...
  if (!user) throw new globalThis.Error('User not found');
...
export async function deductTokens(userId: number, amount: number) {
  ...
  if (!wallet) throw new globalThis.Error('No ad token wallet for user')
  if (wallet.balance < amount) throw new globalThis.Error('Insufficient tokens')
