...
router.get('/overview', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({
      ...
      by_type: byType.map(t => ({ type: t.property_type || 'other', count: t?._count ?? 0 })),
      by_status: byStatus.map(s => ({ status: s.status, count: s?._count ?? 0 })),
...
router.get('/leads', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    res.json({
      ...
      by_priority: byPriority.map(p => ({ priority: p.priority, count: p?._count ?? 0 })),
      by_status: byStatus.map(s => ({ status: s.status, count: s?._count ?? 0 })),
      by_source: bySource.map(s => ({ source: s.source || 'direct', count: s?._count ?? 0 }))
    });
  } catch (error) {
