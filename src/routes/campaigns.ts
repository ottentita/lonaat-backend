...
router.post('/marketplace', [ authMiddleware, body('marketplaceItemId').notEmpty().isInt(), body('budgetTokens').notEmpty().isInt() ], async (req: AuthRequest, res: Response) => {
  ...
  try {
    ...
    const marketplaceItemId = Number(req.body.marketplaceItemId);
    const budgetTokens = Number(req.body.budgetTokens);
    ...
    const result = await prisma.$transaction(async (tx) => {
      await debitTokensTransactional(tx, userId, budgetTokens, 'campaign_launch');
      ...
      await tx.transactionLedger.create({ data: { userId: userId, campaignId: campaign.id, amount: -budgetTokens, type: 'debit', reason: 'campaign_launch' } });
...
router.get('/:id', authMiddleware, async (req: AuthRequest, res: Response) => {
  try {
    ...
    if (!req.user!.isAdmin && campaign.user_id !== userId) {
