...
function calculateListingFee(
  ...
  const fees =
    LISTING_FEES[propertyType as keyof typeof LISTING_FEES] ||
    LISTING_FEES.house;
    ...
    try {
      ...
      if (!title || !property_type || !transaction_type || !price) {
        return res.status(400).json({
          error:
            "Title, property type, transaction type, and price are required",
        });
      }
      ...
      const listingFee = isAdmin
        ? 0
        : calculateListingFee(property_type, transaction_type);
      ...
      const property = await prisma.realEstateProperty.create({
        data: {
          ...
          land_size_sqft: land_size_sqft ? Number(land_size_sqft) : null,
          floors: floors ? Number(floors) : null,
          parking_spaces: parking_spaces ? Number(parking_spaces) : null,
      ...
      res.status(201).json({
        message: isAdmin
          ? "Property created and auto-approved"
          : "Property created. Please pay listing fee.",
        property,
        listing_fee: listingFee,
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      const urls = files.map(
        (f) => `/uploads/property-documents/${f.filename}`,
      );
  ...
  res.json({
    property_types: [
      ...
      },
      {
        ...
        label: "Land / Plot",
        icon: "map",
        fields: ["land_size_sqft"],
      },
      {
        id: "commercial",
        label: "Commercial Property",
        icon: "store",
        fields: ["area_sqft", "floors", "parking_spaces", "condition"],
      },
      {
        id: "guest_house",
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (property.user_id !== userId && !req.user!.isAdmin) {
        return res
          .status(403)
          .json({ error: "You can only edit your own properties" });
      }
      ...
      const updated = await prisma.realEstateProperty.update({
        ...
        data: {
          ...
          bedrooms:
            bedrooms !== undefined ? Number(bedrooms) : property.bedrooms,
          bathrooms:
            bathrooms !== undefined ? Number(bathrooms) : property.bathrooms,
          area_sqft:
            area_sqft !== undefined ? Number(area_sqft) : property.area_sqft,
...
router.delete(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      await prisma.propertyAd.deleteMany({
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (property.user_id !== userId) {
        return res
          .status(403)
          .json({ error: "You can only pay for your own properties" });
      }

      if (property.is_paid) {
        return res
          .status(400)
          .json({ error: "Property listing fee already paid" });
      }
      ...
      if (pendingPayment) {
        return res
          .status(400)
          .json({ error: "A payment is already pending for this property" });
      }
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (property.user_id !== userId && !isAdmin) {
        return res
          .status(403)
          .json({ error: "You can only boost your own properties" });
      }

      if (property.status !== "approved") {
        return res
          .status(400)
          .json({ error: "Only approved properties can be boosted" });
      }
      ...
      if (activeAd) {
        return res
          .status(400)
          .json({ error: "Property already has an active boost" });
      }
      ...
      res.json({
        message: isAdmin
          ? "Property boosted for FREE (admin)"
