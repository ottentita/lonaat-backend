...
import { clickRateLimiter, productClickRateLimiter } from '../middleware/clickRateLimit'
import { antiBotProtection } from '../middleware/antiBotProtection'
import { generateFingerprint, extractFingerprintData, getClientIp } from '../utils/fingerprint'

const router = express.Router()

// 🔥 HARDENED CLICK TRACKING - Production-Ready with Anti-Fraud
router.get('/click/:productId', [
  antiBotProtection,           // Block bots
  clickRateLimiter,            // Rate limit per IP
  productClickRateLimiter      // Rate limit per product
  ...
  try {
    ...
    // � Generate strong fingerprint
    const fingerprintData = extractFingerprintData(req);
    fingerprintData.userId = req.user?.id ? Number(req.user.id) : undefined;
    const fingerprint = generateFingerprint(fingerprintData);

    // �🔥 Track click with duplicate prevention
    try {
      const clickRecord = await prisma.product_clicks.create({
        data: {
      ...
      // Update product click count (only for unique clicks)
      ...
      console.log(`✅ UNIQUE CLICK TRACKED: Product ${productId}, Click ID ${clickRecord.id}, User ${req.user?.id || 'guest'}`);

    } catch (error: any) {
      // Handle duplicate click (unique constraint violation)
      if (error.code === 'P2002') {
        console.log(`⚠️ DUPLICATE CLICK BLOCKED: Product ${productId}, Fingerprint ${fingerprint.substring(0, 8)}...`);
...
// ✅ PHASE 2 - LAYER 2: AUTHENTICATED PRODUCT CLICK TRACKING
// POST /api/track/product-click - Record product clicks for analytics
router.post('/product-click', authMiddleware, async (req: AuthRequest, res) => {
  try {
    ...
    // Generate unique click ID and token
    ...
    // Find or create offer for this product
    ...
    // If no offer exists, create a placeholder offer
    if (!offer) {
      console.log('📦 Creating placeholder offer for product:', productId);
      offer = await prisma.offer.create({
        data: {
          title: `Product ${productId}`,
          description: `Tracked product ${productId}`,
