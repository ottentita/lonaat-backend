...
(0, vitest_1.describe)('Marketplace pipeline', () => {
    ...
    (0, vitest_1.it)('creates marketplace item via API for eligible user', async () => {
        ...
        const slugX = `offerX-${Date.now()}`;
        const offer = await prisma_1.default.offer.create({ data: { title: 'offerX', name: 'offerX', slug: slugX, url: 'https://x', externalOfferId: `eo-${Date.now()}`, trackingUrl: 'https://track.example/clk' } });
    ...
    (0, vitest_1.it)('creates campaign and deducts tokens', async () => {
        ...
        const slugY = `offerY-${Date.now()}`;
        const offer = await prisma_1.default.offer.create({ data: { title: 'offerY', name: 'offerY', slug: slugY, url: 'https://y', externalOfferId: `eo2-${Date.now()}`, trackingUrl: 'https://trk.example/offer' } });
    ...
    (0, vitest_1.it)('click redirect includes aff_id, sub_id and click_token', async () => {
        ...
        const slugZ = `offerZ-${Date.now()}`;
        const offer = await prisma_1.default.offer.create({ data: { title: 'offerZ', name: 'offerZ', slug: slugZ, url: 'https://z', externalOfferId: `eo3-${Date.now()}`, trackingUrl: 'https://redir.example/offer' } });
        const item = await prisma_1.default.marketplaceItem.create({ data: { userId: user.id, offerId: offer.id } });
        const { generateToken } = await Promise.resolve().then(() => __importStar(require('../src/utils/jwt')));
        const t = generateToken({ id: user.id, role: 'user', email: user.email, name: user.name });
        const res = await (0, supertest_1.default)(server)
