...
describe('Marketplace pipeline', () => {
  ...
  it('creates marketplace item via API for eligible user', async () => {
    ...
    const offer = await prisma.offer.create({ data: { title: 'offerX', name: 'offerX', slug: slugX, url: 'https://x', externalOfferId: `eo-${Date.now()}`, trackingUrl: 'https://track.example/clk' } })
  ...
  it('creates campaign and deducts tokens', async () => {
    ...
    const offer = await prisma.offer.create({ data: { title: 'offerY', name: 'offerY', slug: slugY, url: 'https://y', externalOfferId: `eo2-${Date.now()}`, trackingUrl: 'https://trk.example/offer' } })
  ...
  it('click redirect includes aff_id, sub_id and click_token', async () => {
    ...
    const offer = await prisma.offer.create({ data: { title: 'offerZ', name: 'offerZ', slug: slugZ, url: 'https://z', externalOfferId: `eo3-${Date.now()}`, trackingUrl: 'https://redir.example/offer' } })
    const item = await prisma.marketplaceItem.create({ data: { userId: user.id, offerId: offer.id } })
