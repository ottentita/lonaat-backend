...
const VPN_PATTERNS = [
  /vpn/i, /proxy/i, /tor/i, /anonymous/i, /hide/i, /tunnel/i
];

const DATACENTER_ASNS = [
  'Amazon', 'Google', 'Microsoft', 'Digital Ocean', 'Linode', 
  'Vultr', 'OVH', 'Hetzner', 'Cloudflare', 'DigitalOcean'
];
...
function checkVpnIndicators(req: Request): boolean {
  ...
  if (xForwardedFor && typeof xForwardedFor === 'string' && xForwardedFor.split(',').length > 3) {
    return true;
  }
...
export async function analyzeClickFraud(
  ...
  if (recentClicksSameProduct >= 5) {
    return { fraudulent: true, score: 100, reason: 'Excessive clicks on same product in short period' };
  }
  ...
  if (recentClicksSameIp >= 50) {
    return { fraudulent: true, score: 90, reason: 'Excessive clicks from same IP' };
  }
  ...
  if (signals.isVpn || signals.isTor || signals.isProxy) {
    return { fraudulent: false, score: 40, reason: 'VPN/Proxy detected - flagged for review' };
  }
...
export async function detectAdvancedFraud(
  ...
  if (!user) {
    return { score: 0, reasons: ['User not found'], shouldBlock: false, signals };
  }
  ...
  const uniqueFingerprints = new Set(
    recentLogs.map(l => (l.details as any)?.device_fingerprint).filter(Boolean)
  );
  if (uniqueFingerprints.size > 5) {
    score += 30;
    reasons.push(`Multiple device fingerprints: ${uniqueFingerprints.size} devices in 24h`);
  }
  ...
  if (signals.geo?.country) {
    ...
    if (uniqueCountries.size > 3) {
      score += 40;
      reasons.push(`Impossible travel: ${uniqueCountries.size} countries in 24h`);
    }
  }
...
async function checkSelfReferral(userId: number, ip: string): Promise<{ isSelfReferral: boolean; reason?: string }> {
