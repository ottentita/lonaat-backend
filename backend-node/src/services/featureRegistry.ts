...
export function registerFeature(feature: Feature) {
  ...
  registerFeatureConfig({ name: feature.name, action: feature.name as any, minPlan: 'basic', tokenCost: feature.tokenCost, description: feature.description })
}

export function getFeature(name: string): Feature | undefined {
  const cfg = getFeatureConfig(name)
  if (!cfg) return undefined
  return { name: cfg.name, tokenCost: cfg.tokenCost, description: cfg.description }
}

export function listFeatures(): Feature[] {
  return listAIRegistered().map(f => ({ name: f.name, tokenCost: f.tokenCost, description: f.description }))
}
