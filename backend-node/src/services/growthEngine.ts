...
export async function rankProducts(
  ...
  const conversionsMap = new Map(
    conversions.map((c) => [
      c.product_id,
      { count: c._count.id, amount: c._sum.amount || 0 },
    ]),
  );
  const socialMap = new Map(
    socialPosts.map((s) => [s.product_id, s._count.id]),
  );
  ...
  const scoredProducts: ProductScore[] = products.map((product) => {
    ...
    const conversionRate =
      clickCount > 0 ? (conversionData.count / clickCount) * 100 : 0;
    ...
    if (socialCount >= 3) {
      ...
      recommendations.push(
        `Only ${socialCount} social posts - consider expanding`,
      );
    }
...
export async function generateGrowthInsights(
  ...
  const approvedCommissions = user.commissions.filter(
    (c) => c.status === "approved" || c.status === "paid_by_network",
  );
  const approvalRate =
  ...
  if (approvalRate < 50 && user.commissions.length > 5) {
    insights.push({
      ...
      actionItem:
        "Focus on higher-quality traffic sources and target audience refinement",
      priority: "high",
...
export async function analyzeROI(userId: number): Promise<{
  ...
  const currentROI =
    estimatedEffort > 0 ? (totalEarnings / estimatedEffort) * 100 : 0;
  ...
  if (currentROI < 50) {
    recommendations.push(
      "Focus on higher-commission products (>10% commission rate)",
    );
...
export async function generateGrowthReport(
  ...
  const [topProducts, platformOptimizations, growthInsights, roiAnalysis] =
    await Promise.all([
...
function calculateOverallScore(
  ...
  const highPriorityIssues = insights.filter(
    (i) => i.priority === "high",
  ).length;
...
Top Products: ${report.topProducts
      ...
      .map((p) => p.name)
      .join(", ")}
Key Issues: ${report.growthInsights
      .slice(0, 3)
      .map((i) => i.insight)
      .join("; ")}
