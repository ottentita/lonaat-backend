...
export const SUPPORTED_CURRENCIES = ['USD', 'XAF', 'EUR', 'USDT', 'BTC', 'ETH'] as const;
export type SupportedCurrency = typeof SUPPORTED_CURRENCIES[number];

export const PAYOUT_PROVIDERS = {
  payoneer: { name: 'Payoneer', currencies: ['USD', 'EUR'], minAmount: 20, fee: 2 },
  mtn_momo: { name: 'MTN Mobile Money', currencies: ['XAF'], minAmount: 5000, fee: 1.5, countries: ['CM', 'CI', 'SN', 'GH'] },
  orange_money: { name: 'Orange Money', currencies: ['XAF'], minAmount: 5000, fee: 1.5, countries: ['CM', 'CI', 'SN', 'ML'] },
  usdt_trc20: { name: 'USDT (TRC20)', currencies: ['USDT'], minAmount: 10, fee: 1 },
  usdt_erc20: { name: 'USDT (ERC20)', currencies: ['USDT'], minAmount: 50, fee: 5 },
  btc: { name: 'Bitcoin', currencies: ['BTC'], minAmount: 0.001, fee: 0.0001 },
  eth: { name: 'Ethereum', currencies: ['ETH'], minAmount: 0.01, fee: 0.005 },
  bank_transfer: { name: 'Bank Transfer', currencies: ['USD', 'EUR', 'XAF'], minAmount: 50, fee: 5 }
} as const;
...
export async function getWalletBalance(userId: number, currency: SupportedCurrency): Promise<number> {
...
export async function getAllWalletBalances(userId: number): Promise<Record<SupportedCurrency, { balance: number; pending: number; locked: number }>> {
  const balances = await prisma.walletBalance.findMany({
    where: { user_id: userId }
  });

  const result: Record<string, { balance: number; pending: number; locked: number }> = {};
  ...
  return result as Record<SupportedCurrency, { balance: number; pending: number; locked: number }>;
}
...
export async function lockBalance(userId: number, currency: SupportedCurrency, amount: number): Promise<boolean> {
  const balance = await prisma.walletBalance.findUnique({
    where: { user_id_currency: { user_id: userId, currency } }
  });
...
export async function unlockBalance(userId: number, currency: SupportedCurrency, amount: number): Promise<void> {
  await prisma.walletBalance.update({
    where: { user_id_currency: { user_id: userId, currency } },
    data: {
      balance: { increment: amount },
...
export async function deductLockedBalance(userId: number, currency: SupportedCurrency, amount: number): Promise<void> {
  await prisma.walletBalance.update({
    where: { user_id_currency: { user_id: userId, currency } },
    data: {
      locked: { decrement: amount }
...
export async function runFraudCheck(userId: number, amount: number, currency: SupportedCurrency): Promise<FraudCheckResult> {
  const flags: string[] = [];
  let score = 0;
  ...
  if (!user) {
    return { score: 100, flags: ['user_not_found'], blocked: true, reason: 'User not found' };
  }

  if (user.is_blocked) {
    return { score: 100, flags: ['user_blocked'], blocked: true, reason: 'Account is blocked' };
  }
  ...
  const accountAgeHours = (Date.now() - user.created_at.getTime()) / (1000 * 60 * 60);
...
export async function createPayout(request: PayoutRequest): Promise<PayoutResult> {
  ...
  if (!payoutMethod) {
    return { success: false, error: 'Payout method not found', requiresApproval: false };
  }

  if (!payoutMethod.is_verified) {
    return { success: false, error: 'Payout method not verified', requiresApproval: false };
  }

  const balance = await getWalletBalance(userId, currency);
  if (balance < amount) {
    return { success: false, error: 'Insufficient balance', requiresApproval: false };
  }
  ...
  if (!locked) {
    return { success: false, error: 'Failed to lock balance', requiresApproval: false };
  }
  ...
  try {
    const payout = await prisma.payout.create({
      data: {
        ...
        provider,
        provider_fee: providerConfig ? new Prisma.Decimal(providerConfig.fee) : null,
...
export async function approvePayout(payoutId: number, adminId: number, adminName: string): Promise<{ success: boolean; error?: string }> {
...
export async function rejectPayout(payoutId: number, adminId: number, reason: string): Promise<{ success: boolean; error?: string }> {
  const payout = await prisma.payout.findUnique({
    where: { id: payoutId }
  });
  ...
  await unlockBalance(payout.user_id, payout.currency as SupportedCurrency, Number(payout.amount));
...
export async function processPayout(payoutId: number): Promise<{ success: boolean; providerRef?: string; error?: string }> {
  const payout = await prisma.payout.findUnique({
    where: { id: payoutId },
  ...
  try {
    ...
    await deductLockedBalance(payout.user_id, payout.currency as SupportedCurrency, Number(payout.amount));
...
async function processPayoneerPayout(payout: any): Promise<string> {
  console.log(`Processing Payoneer payout: ${payout.id}, Amount: ${payout.amount} ${payout.currency}`);
...
async function processMobileMoneyPayout(payout: any): Promise<string> {
  ...
  console.log(`Processing ${payout.provider} payout to ${method.mobile_number}`);
...
export async function getPayoutHistory(userId: number, options: {
  status?: string;
  limit?: number;
  offset?: number;
} = {}): Promise<any[]> {
...
export async function getPendingPayoutsForAdmin(options: {
  limit?: number;
  offset?: number;
} = {}): Promise<any[]> {
